...
func SaveFiles(files []*multipart.FileHeader, saveFunc func(*multipart.FileHeader, string) error, lastInsertedRow int64) error {
	for _, file := range files {
		...
		dst := fmt.Sprintf("%s/bills/%s/%s", DocumentsPath, lastInsertedRow, file.Filename)
